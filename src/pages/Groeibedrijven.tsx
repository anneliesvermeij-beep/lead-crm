...
export function Groeibedrijven() {
  const alle = useLeads();
    ...
    <div className="pagina">
      <header className="app-kop">
        <div className="app-kop-titel">
          <span className="studio-naam">Groeibedrijven</span>
          <span className="app-kop-sub">Snelle groeiers met zwakke fotografie</span>
        </div>
      ...
      <p className="rustige-tekst" style={{ marginBottom: 12 }}>
        Snelgroeiende of succesvolle bedrijven (o.a. FD Gazellen) waarvan de
        website zwakke fotografie heeft — geld, groei én behoefte aan beter beeld.
        Ze staan los van je bureau-leads.
      </p>
      ...
      {storeStatus === 'laden' && alle.length === 0 ? (
        ...
        <div className="leeg-vlak">
          <p>Nog geen groeibedrijven in de lijst.</p>
        </div>
      ) : (
        <>
          ...
          <ul className="lijst lijst-compact">
            {resultaat.map((lead) => {
              const reden = lead.contactMomenten[0]?.notitie ?? '';
              return (
                <li
                  key={lead.id}
                ...
                >
                  ...
                  <div className="rij-midden">
                    ...
                    {reden && <span className="rij-reden">{reden}</span>}
                  </div>
